...
@Component({
  selector: 'app-view-by-location',
  templateUrl: './view-by-location.component.html',
  styleUrls: ['./view-by-location.component.css']
})
export class ViewByLocationComponent implements OnInit {
  location!: string;
  ...
  constructor(private route: ActivatedRoute, private offerService: OfferServicesService) { }
  ...
  ngOnInit(): void {
    // Récupérer la location à partir de l'URL
    const locationParam = this.route.snapshot.queryParamMap.get('location');

    // Vérifier si locationParam n'est pas null avant de l'assigner à location
    if (locationParam !== null) {
      this.location = locationParam;

      // Appeler le service pour obtenir les offres par location
      this.offerService.getOffersByLocation(this.location).subscribe(
        (offers: OfferModuleModule[]) => {
          this.offers = offers;
          this.loading = false;
        },
      ...
      console.error('Location parameter is null.');
    }
  }
}
