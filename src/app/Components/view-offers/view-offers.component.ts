...
import {FavOffersModule} from "../../Core/Modules/fav-offers/fav-offers.module";
...
export class ViewOffersComponent implements OnInit{
  ...
  favorites: FavOffersModule[] = [];
    ...
    constructor(private offerService: OfferServicesService , private route : ActivatedRoute ,private router: Router , private snackBar: MatSnackBar) { }
  ...
  ngOnInit(): void {
    ...
    this.loadFavorites();
    this.offerLink = "http://localhost:4200"+this.router.url;
  }
  ...
  loadFavorites() {
    this.offerService.getFavOffers().subscribe(
      ...
      (error) => {
        console.log('Error fetching favorite offers:', error);
      }
    );
  }

  addToFavorites(offer: OfferModuleModule) {
    // Vérifier si l'offre est déjà dans la liste des favoris
    if (this.isFavoriteAlready(offer)) {
      this.openSnackBar('Offer Already Added To Favourites');
    } else {
      this.offerService.addToFav(offer).subscribe(
        () => {
          // Actualiser les favoris après l'ajout
          this.loadFavorites();
          console.log("added");
        },
        (error) => {
          console.log('Error adding offer to favorites:', error);
        }
      );
    }
  }
  openSnackBar(message: string) {
    this.snackBar.open(message, 'Close', {
      duration: 3000, // Durée pendant laquelle le snack bar est affiché en millisecondes
      verticalPosition: 'top' // Position verticale du snack bar (par exemple, 'top', 'bottom')
      // Vous pouvez également spécifier d'autres options comme horizontalPosition, panelClass, etc., selon vos besoins
    });
  }

  isFavorite(offer: OfferModuleModule): boolean {
    return this.favorites.some(favorite => favorite.idOffer === offer.idOffer);
  }
  isFavoriteAlready(offer: OfferModuleModule): boolean {
    return this.favorites.some(favorite => favorite.offerTitle === offer.offerTitle);
  }


  toggleFavorite(offer: OfferModuleModule): void {
    if (this.isFavorite(offer)) {
      if (offer.idOffer !== undefined) {
        this.offerService.removeFavOffer(offer.idOffer).subscribe(
          () => {
            // Actualiser les favoris après la suppression
            this.loadFavorites();
          },
          (error) => {
